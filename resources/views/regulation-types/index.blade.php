                        ...
                        <tr>
                            ...
                            <td>
                                <div class="flex items-center justify-end gap-2">
                                    <x-button href="{{ route('regulation-types.edit', $type) }}" variant="outline"
                                        size="sm">
                                    ...
                                    {{-- <form method="POST" action="{{ route('regulation-types.destroy', $type) }}"
                                        id="delete-type-form-{{ $type->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <x-button type="button" variant="danger" size="sm"
                                            onclick="window._deleteTypeId={{ $type->id }}"
                                            @click="$dispatch('open-modal-confirm-delete-type')">
                                    ...
                                    <form method="POST" action="{{ route('regulation-types.toggle', $type) }}">
                                        @csrf
                                        @method('PATCH')
                                        <x-button type="submit" variant="{{ $type->is_active ? 'danger' : 'success' }}"
                                            size="sm">
                                            {{ $type->is_active ? 'Nonaktifkan' : 'Aktifkan' }}
                                        </x-button>
                                    </form>
                                    <a href="{{ route('regulation-types.show', $type) }}"
